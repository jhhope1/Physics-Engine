...
public:
	...
	static int Collision_Triangle_Point(int indA, int indB, Object *A, Object *B, double dt);
	static void Collision_Line_Line(int indA, int indB, Object A, Object B, double dt);
	static void GenIndexPointForce_f(vector <Object> OB, double dt);
	static void Gravity(int ind, Object *ob);
	static void Gravity_Object(vector<Object> OB);
	static void update_Object(Object* ob, vec workingpoint, vec force, double dt);

	static vector<vec> Force_f(vector <Object> OB);//지금은 수동 나중에는 바꿔줘야함
